...
/**
 * A helper class for managing player loot data.
 */
public class PlayerLootDataHelper {

    /**
     * Handles the addition of stackable loot to the player's current loot list.
     * If the item can be stacked, it attempts to combine it with existing stacks.
     * Otherwise, it creates new stacks until the item count is exhausted.
     *
     * @param currentPlayerLoot The current loot of the player.
     * @param eventStack The ItemStack being processed.
     * @param player The player interacting with the loot.
     */
    public static void handleStackableLoot(List<ItemStack> currentPlayerLoot, ItemStack eventStack, PlayerEntity player) {
    ...
    /**
     * Adds a new item stack to the player's loot if it is considered valuable.
     * The item is only added if it is not already present in the player's loot.
     *
     * @param player The player to whom the item is being added.
     * @param stack The ItemStack to be added to the player's loot.
     */
    public static void addNewItemToPlayerLoot(PlayerEntity player, ItemStack stack) {
