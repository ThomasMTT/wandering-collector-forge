...
/**
 * Helper class for calculating the price of item stacks based on various criteria.
 */
public class StackPriceCalculator {
    ...
    /**
     * Calculates the price of the given item stack.
     *
     * @param stack The item stack to calculate the price for.
     * @return The calculated price for the item stack.
     */
    public static int getStackPrice(ItemStack stack) {
        ...
        if (configPrice != -1) {
            ...
            if (isEnchanted(stack)) {
                price += stack.getAllEnchantments().size() * basePrice * 2;
            }
            return price;
        }
