...
public class ItemEntityUtil {
    ...
    /**
     * Checks if the given ItemEntity has despawned based on certain conditions.
     *
    ...
    public static boolean itemEntityHasDespawned(ItemEntity itemEntity, int minBuildHeight) {
        return (itemEntity.hurtMarked || itemEntity.lifespan == itemEntity.tickCount ||
                itemEntity.getY() < minBuildHeight ||
                (itemEntity.isOnFire()) && !itemEntity.fireImmune());
    }
}
